...
public class TimesheetRow implements java.io.Serializable {
     ...
     */
    public TimesheetRow(final String id, final String wp,
            final BigDecimal[] hours, final String comments, String labourGrade) {
        setProjectID(id);
        setWorkPackage(wp);
        setHoursForWeek(hours);
        setNotes(comments);
        setLabourGradeID(labourGrade); //--
    }
        ...
        if (hour != null) {
            if (hour.compareTo(Timesheet.HOURS_IN_DAY) > 0.0
                ...
                throw new IllegalArgumentException(
                       "Hours out of range: should be between 0 and 24");
            }
            if (hour.scale() > 1) {
                throw new IllegalArgumentException(
